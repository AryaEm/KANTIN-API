...
const createTransaksi = async (req, res) => {
    try {
        ...
        const transaksi = await prisma_1.prisma.transaksi.create({
            data: {
                id_stan: stanId,
                id_siswa: siswa.id,
                detail: {
        ...
        await prisma_1.prisma.transaksi.update({
...
const updateStatus = async (req, res) => {
    try {
        ...
        const transaksi = await prisma_1.prisma.transaksi.findUnique({
        ...
        const updated = await prisma_1.prisma.transaksi.update({
            where: { id: id_transaksi },
            data: { status },
        });
...
const deleteOrder = async (req, res) => {
    try {
        ...
        const transaksi = await prisma_1.prisma.transaksi.findFirst({
            where: {
                id: id_transaksi,
                id_siswa: siswa.id,
            },
        ...
        await prisma_1.prisma.transaksi.delete({
            where: { id: transaksi.id },
        });
        return res.json({
            status: true,
...
const getStanHistory = async (req, res) => {
    try {
        const authUser = res.locals.user;
        if (!authUser) {
            return res.status(401).json({
        ...
        const transaksiList = await prisma_1.prisma.transaksi.findMany({
            where: {
                id_stan: stan.id,
                status: {
...
const getStanHistorySelesai = async (req, res) => {
    try {
        const authUser = res.locals.user;
        if (!authUser) {
            return res.status(401).json({
...
const getSiswaHistory = async (req, res) => {
    try {
        const authUser = res.locals.user;
        if (!authUser) {
            return res.status(401).json({
...
const getIncome = async (req, res) => {
    try {
        ...
        const transaksi = await prisma_1.prisma.transaksi.findMany({
            where: {
                id_stan: stan.id,
                status: "selesai",
                ...(startDate &&
...
const getOrder = async (req, res) => {
    try {
        ...
        const transaksi = await prisma_1.prisma.transaksi.findMany({
            where: {
                id_stan: stan.id,
                status: client_1.StatusTransaksi.selesai,
                ...(startDate &&
                    endDate && {
                    tanggal: {
                        gte: startDate,
...
const getPendingTransactionCount = async (req, res) => {
    try {
        const authUser = res.locals.user;
        if (!authUser) {
            return res.status(401).json({
        ...
        const pending_count = await prisma_1.prisma.transaksi.count({
            where: {
                id_stan: stan.id,
