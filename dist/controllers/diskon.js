...
const getDiskonByStan = async (req, res) => {
    try {
        ...
        const stan = await prisma_1.prisma.stan.findUnique({
            where: { id: idStan },
        });
        if (!stan) {
        ...
        const now = new Date();
        const diskon = await prisma_1.prisma.diskon.findMany({
            where: {
...
const getAllDiskon = async (req, res) => {
    try {
        ...
        const stan = await prisma_1.prisma.stan.findFirst({
...
const getActiveDiskon = async (req, res) => {
    try {
        const authUser = res.locals.user;
        if (!authUser) {
            return res.status(401).json({
        ...
        const now = new Date();
        const diskonAktif = await prisma_1.prisma.diskon.findMany({
            where: {
                tanggal_awal: { lte: now },
                tanggal_akhir: { gte: now },
            },
...
const createDiskon = async (req, res) => {
    try {
        const authUser = res.locals.user;
        if (!authUser) {
            return res.status(401).json({
        ...
        const diskon = await prisma_1.prisma.diskon.create({
            data: {
...
const getAvailableDiskon = async (req, res) => {
    try {
        const authUser = res.locals.user;
        if (authUser.role !== "admin_stan") {
            return res.status(403).json({
...
const updateDiskon = async (req, res) => {
    try {
        const authUser = res.locals.user;
        if (!authUser) {
            return res.status(401).json({
        ...
        const diskon = await prisma_1.prisma.diskon.findFirst({
            where: {
                id: diskonId,
                id_stan: stan.id,
            },
        ...
        const tanggalAwalFinal = req.body.tanggal_awal ?? diskon.tanggal_awal;
        const tanggalAkhirFinal = req.body.tanggal_akhir ?? diskon.tanggal_akhir;
        ...
        // 5. Update diskon
        const updatedDiskon = await prisma_1.prisma.diskon.update({
            where: {
                id: diskonId,
            },
            data: dataToUpdate,
...
const getDiskonStatus = async (req, res) => {
    try {
        const authUser = res.locals.user;
        if (!authUser) {
            return res.status(401).json({
        ...
        const diskonList = await prisma_1.prisma.diskon.findMany({
            where: {
                id_stan: stan.id,
            },
            orderBy: {
...
const deleteDiskon = async (req, res) => {
    try {
        const authUser = res.locals.user;
        if (!authUser) {
            return res.status(401).json({
        ...
        await prisma_1.prisma.diskon.delete({
            where: { id: diskonId },
        });
        return res.status(200).json({
            status: true,
...
const pasangDiskon = async (req, res) => {
    try {
        const authUser = res.locals.user;
        if (!authUser) {
            return res.status(401).json({
        ...
        const existing = await prisma_1.prisma.menuDiskon.findFirst({
            where: {
                id_menu: idMenu,
                id_diskon: idDiskon,
            },
        ...
        await prisma_1.prisma.menuDiskon.create({
            data: {
                id_menu: idMenu,
                id_diskon: idDiskon,
            },
...
const lepasDiskon = async (req, res) => {
    try {
        const authUser = res.locals.user;
        if (!authUser) {
            return res.status(401).json({
        ...
        const menuDiskon = await prisma_1.prisma.menuDiskon.findFirst({
            where: {
                id_menu: idMenu,
                id_diskon: idDiskon,
            },
        ...
        await prisma_1.prisma.menuDiskon.delete({
            where: {
                id: menuDiskon.id,
            },
        });
